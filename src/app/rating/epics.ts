...
export class RatingEpics {
  constructor(
    ...
    private actions: RatingAPIActions,
  ) { }
  ...
  private createLoadEpic(): Epic<
    RatingAPIAction<IRating[] | IRatingError>,
    RatingAPIAction<IRating[] | IRatingError>,
    AppState
  > {
    return (action$, state$) =>
      action$.pipe(
        filter((a) => a.type === RatingAPIActions.LOAD),
        filter((a) => countriesNotAlreadyFetched(a.meta.country, state$.value)),
        switchMap((a) =>
          this.service.getAll(a.meta.country).pipe(
            map(data => this.actions.loadSucceeded(data, a.meta.country)),
            catchError(res => of(
              this.actions.loadFailed({
                status: '' + res.status,
              }, a.meta.country),
            )),
            startWith(this.actions.loadStarted(a.meta.country)))
        ));
  }
}